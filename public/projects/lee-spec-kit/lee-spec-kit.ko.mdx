...
keyFeatures:
  ...
  - 에이전트 컨텍스트·문서 검증과 OpenWiki 기반 온보딩 문서 동기화
links:
...
Feature 단위로 스펙과 구현 계획, 결정 기록을 남기다 보니 변경의 의도와 이력은 추적하기 쉬워졌지만, 시간이 지나면서 프로젝트 전체의 현재 모습을 파악하려면 여러 Feature 문서와 코드를 오가며 읽어야 했습니다.

이를 보완하기 위해 OpenWiki를 연동해 현재 코드베이스를 중심으로 구조와 주요 흐름을 설명하는 온보딩 자료를 생성하도록 했습니다. 필요한 경우 기존 스펙과 결정 기록도 함께 참고해 현재 구현의 배경을 이해할 수 있도록 합니다.

기능이 변경되면 OpenWiki 문서도 함께 동기화하고 근거를 검증해, 과거의 변경 기록과 현재의 구현 상태가 뒤섞이지 않도록 합니다. 이렇게 생성된 문서는 새로운 개발자와 에이전트가 현재 프로젝트를 이해하고 필요한 코드와 문서를 탐색하기 위한 출발점으로 활용할 수 있습니다.
...
- OpenWiki 연동: `knowledge doctor`, `sync`, `audit`로 준비 상태·문서 동기화·검증을 수행합니다. 활성화 시 Feature 리뷰 전에 동기화와 전용 Knowledge 커밋을 요구하며, 내장 기술 문서 작성 스킬로 생성 문서의 작성 기준을 유지합니다.
