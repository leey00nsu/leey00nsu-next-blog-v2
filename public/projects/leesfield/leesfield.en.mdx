...
keyFeatures:
  - A unified generation environment for running multiple AI models through one interface
  - History and monitoring that keep generated results together with prompts and settings
  - Extensible AI generation through node-based workflows and a unified API
links:
...
Users select and run image, video, and audio models from one generation screen and manage results alongside their prompts and settings in the history. A model catalog manages inputs and defaults, while provider-specific adapters handle generation requests.
**Spaces**, a node-based workspace, connects inputs, generation, editing, and outputs and lets users run individual nodes. External APIs and Leemage storage integration make generation features and results available to other services.

## Key Features

- AI image, video, and audio generation: A unified screen for selecting media and models with model-specific inputs and results
- Spaces: Node-based workflow composition and individual node execution
...
- Generation history management: Save, browse, and reuse results with their prompts and settings
