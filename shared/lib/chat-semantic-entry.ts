...
import type { ChatSemanticEntry } from '@/shared/model/chat-semantic-map'
...
export function collectUniqueChatSemanticValues(values: string[]): string[] {
  return [
...
function extractPrimaryProfileName(content: string): string[] {
  ...
  return collectUniqueChatSemanticValues([headingName])
}
...
export function buildProfileChatSemanticEntry({
  ...
  return {
    ...
    aliases: collectUniqueChatSemanticValues([
      title,
      description ?? '',
      ...resolveProfileAliases(locale),
    ]),
...
export function buildProjectChatSemanticEntry({
  ...
  return {
    ...
    entityNames: collectUniqueChatSemanticValues([title]),
    aliases: collectUniqueChatSemanticValues([
      title,
      summary,
      ...keyFeatures,
      ...
      queryIntents: [
        ...
        },
        {
          ...
          subjectTerms: techStacks
            .slice(0, CHAT_SEMANTIC_ENTRY.QUERY_LIMIT_PER_TAG)
...
export function buildPostChatSemanticEntry({
  ...
  return {
    ...
    sourceCategory: 'blog',
    entityNames: collectUniqueChatSemanticValues([title]),
    aliases: collectUniqueChatSemanticValues([title, description, ...tags]),
    faqQueries: buildFaqQueries({
      locale,
      queryIntents: [
        {
